...
//    animation.fillMode = kCAFillModeRemoved;
    ...
    // 动画执行完毕后是否从图层上移除，默认为YES. 若移除,则图形会恢复到动画执行前的状态。如果想让图层保持显示动画执行后的状态，那就设置为NO，不过还要设置fillMode属性为kCAFillModeForwards
...
//    animation.byValue = [NSNumber numberWithFloat:0.5];
    ...
    // 2. 方法addAnimation:forKey:是将 CABasicAniamtion 对象进行了 copy 操作的。所以在将其添加到一个layer上之后，我们还是可以将其再次添加到另一个layer上的。
...
- (void)animationDidStart:(CAAnimation *)anim {
    // Note:把动画存储为一个属性然后再回调中比较，用来判定是哪个动画的方法是不可行的,因为委托传入的动画参数是原始值的一个深拷贝，不是同一个值.
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag {
    // flag参数表明了动画是自然结束还是被打断的,比如调用了removeAnimationForKey:方法或removeAnimationForKey方法，flag为NO，如果是正常结束，flag为YES。
    
    // 此时,不能再使用[self.animationImageView.layer animationForKey:@"TransformScale"]来区分不同的动画了,
    // 因为动画完成时已经把CAAnimation从layer上移除了,所以[ animationForKey:]方法获取到的CAAnimation会一直为nil,不能用于判断.
